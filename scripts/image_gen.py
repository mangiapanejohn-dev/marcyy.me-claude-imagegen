#!/usr/bin/env python3
"""CLI for image generation and editing with GPT Image models, for the
imagegen Claude Code skill.

Adapted from Codex CLI's built-in imagegen skill (Apache 2.0); this is Claude
Code's only execution path (there is no separate built-in tool to defer to).
...
def _ensure_api_key(dry_run: bool) -> None:
    if os.getenv("OPENAI_API_KEY"):
        ...
        return
    stored = _key_store.load_key()
    if stored:
        os.environ["OPENAI_API_KEY"] = stored
        base_url = _key_store.load_base_url()
        if base_url:
            os.environ.setdefault("OPENAI_BASE_URL", base_url)
            print(f"Using stored API key + base URL ({base_url}).", file=sys.stderr)
        else:
            print(f"Using stored API key ({_key_store.CONFIG_FILE}).", file=sys.stderr)
        return
    if dry_run:
        _warn("No API key configured; dry-run only.")
        return
    _die(
        "No OpenAI API key configured. Run `python configure_key.py set` "
        "(see SKILL.md's Setup section), or export OPENAI_API_KEY."
    )
