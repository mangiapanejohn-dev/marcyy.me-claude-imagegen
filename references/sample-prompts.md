...
These prompt recipes are for `scripts/image_gen.py`, this skill's only image-generation path.
...
Execution details such as CLI flags, `quality`, `input_fidelity`, masks, output formats, and local output paths are documented in `references/image-api.md`. For transparent images, prompt for a flat chroma-key background and remove it locally with `python scripts/remove_chroma_key.py`; only use true CLI transparency (`gpt-image-1.5`) when the user explicitly opts into it or confirms chroma-key removal isn't working.

Model notes:
- `gpt-image-2` is the default model for new workflows.
- `gpt-image-2` supports `quality` values `low`, `medium`, `high`, and `auto`.
- For 4K-style `gpt-image-2` output, use `3840x2160` or `2160x3840`.
- If transparent output needs true native transparency, ask before using `gpt-image-1.5` unless the user already explicitly requested it. Explain that chroma-key removal is the default path, but `gpt-image-2` does not support `background=transparent`.
