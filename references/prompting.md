...
These prompting principles apply to every call made through `scripts/image_gen.py`, this skill's only image-generation path.

This file is about prompt structure, specificity, and iteration. Execution controls such as `quality`, `input_fidelity`, masks, output format, and output paths live in `references/image-api.md`.
...
## Transparent images
- Default path: generate on a flat chroma-key background with `gpt-image-2`, then strip it locally. If the subject is clearly too complex for chroma-key removal (hair, fur, glass, smoke, liquids, translucent materials, reflections, soft shadows), explain that and ask before switching models.
...
- Use `--model gpt-image-1.5 --background transparent --output-format png` only after the user explicitly confirms true native transparency, or when they already explicitly requested `gpt-image-1.5`. Ask first for true/native transparency requests, failed chroma-key validation, or complex transparent subjects.

## Execution controls
- `quality`, `input_fidelity`, explicit masks, output format, and output paths are documented in `references/image-api.md`.
